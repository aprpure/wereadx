        ...
        start: async (controller) => {
            try {
                ...
                for (const chapter of chapters) {
                    ...
                    // }
                    // 随机思考概率（0-1之间的随机数）
                    const thinkingProbability = Math.random(); // 生成0到1之间的随机数
                    const readingSpeed = randomInteger(400, 600); // 每分钟阅读字符数在400到600之间
                    const readingTime = Math.max(
                        8000, // 最少8秒
                        Math.min(
                            60000, // 最多25秒 
                            html.length / readingSpeed * 30000 // 根据随机阅读速度计算时间
                        )
                    );
                    await randomDelay(readingTime * 0.8, readingTime * 1.2);

                    // 随机暂停,模拟用户思考或休息
                    if (thinkingProbability < 0.3) { // 20%概率
                        await randomDelay(15000, 30000);
                    }
